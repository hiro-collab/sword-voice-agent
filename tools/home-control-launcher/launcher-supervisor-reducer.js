...
const WORKER_EXECUTABLE_CLASSES = new Set(['powershell_7_program_files', 'windows_powershell_system32'])
...
const validatePlanIdentity = (identity) => {
  if (!identity || typeof identity !== 'object' || Array.isArray(identity) ||
      Object.keys(identity).sort().join(',') !== 'private_plan_sha256,worker_executable_class,worker_executable_sha256' ||
      typeof identity.private_plan_sha256 !== 'string' || !SHA256.test(identity.private_plan_sha256) ||
      !WORKER_EXECUTABLE_CLASSES.has(identity.worker_executable_class) ||
      typeof identity.worker_executable_sha256 !== 'string' || !SHA256.test(identity.worker_executable_sha256)) {
    fail('operation_plan_identity_invalid')
  }
  return identity
}

const validateProbeConfigSha256 = (probeConfigSha256) => {
  if (typeof probeConfigSha256 !== 'string' || !SHA256.test(probeConfigSha256)) {
    fail('operation_probe_config_identity_invalid')
  }
  return probeConfigSha256
}

const createOperation = (operationId, authority, configIdentity, planIdentity, probeConfigSha256, supervisorGeneration = 1) => {
  ...
  validateProbeConfigSha256(probeConfigSha256)
...
const startOperation = (active, operationId, authority, configIdentity, planIdentity, probeConfigSha256, supervisorGeneration = 1) => {
  assertAuthority(authority)
  validateIdentityInputs(operationId, authority.identities.graphSha256, authority.identities.bindingSha256)
  validateConfigIdentity(configIdentity, authority)
  validatePlanIdentity(planIdentity)
...
const completeSemanticProbe = (operation, event, authority) => {
  ...
  if (!service || !spec || service.probe_status !== 'transport_ready' || !pendingMatches(operation, event, ['probe']) ||
      ...
      result.service_id !== service.service_id || result.probe_id !== spec.readiness.probe_id ||
      result.config_sha256 !== operation.probe_config_sha256) return invalid(operation)
...
const dependenciesReady = (operation, spec, authority) => spec.dependencies.every((dependencyId) => {
  ...
  return [SERVICE.EXTERNAL_READY, SERVICE.OPTIONAL_ABSENT].includes(state)
})
...
const canOptionalAbsent = (operation, serviceId, authority) => {
  const spec = specOf(authority, serviceId)
  return Boolean(spec && ['optional', 'external'].includes(spec.requirement) &&
    stateOf(operation, serviceId) === SERVICE.PENDING && dependenciesReady(operation, spec, authority))
}
...
const validateSnapshot = (operation, authority) => {
  ...
  validateProbeConfigSha256(operation.probe_config_sha256)
  ...
  for (const service of operation.services) {
    if (service.last_probe_result !== null) {
      ...
      if (service.last_probe_result.operation_id !== operation.operation_id ||
          ...
          service.last_probe_result.config_sha256 !== operation.probe_config_sha256 ||
  ...
  for (const [serviceId, state] of states) {
    ...
    if (state === SERVICE.OPTIONAL_ABSENT && !['optional', 'external'].includes(spec.requirement)) fail('operation_store_record_invalid')
    if (spec.ownership === 'external') {
      if (![SERVICE.PENDING, SERVICE.OPTIONAL_ABSENT, SERVICE.EXTERNAL_READY, SERVICE.FAILED].includes(state) || residueIds.has(serviceId)) fail('operation_store_record_invalid')
