...
'''
    ...
    def test_launcher_readme_documents_fast_timing_and_no_live_diagnostics(self) -> None:
        ...
        self.assertIn("launcher_startup_timing.v1", readme)
        self.assertIn("expected-versus-ready service state", readme)
        self.assertIn("`expectedServiceIds`", readme)
        self.assertIn("`readyServiceIds`", readme)
        self.assertIn("`operational`", readme)
        self.assertIn("`readiness_authority`", readme)
        self.assertIn("`raw_private_publication_flags`", readme)
        self.assertIn("not a measured field from this endpoint", readme)
        self.assertNotIn("launcher_startup_timing.v0", readme)
        self.assertNotIn("first-ready elapsed milliseconds", readme)
        self.assertNotIn("waiting elapsed milliseconds", readme)
        self.assertNotIn("timeline events, and the current critical-path", readme)
        ...
        self.assertIn("one same-origin", readme)
        self.assertIn("only one page executes an intent", readme)
        self.assertIn("fails closed", readme)
        self.assertIn("does not become another effect-host candidate", readme)
        self.assertNotIn("single production effect receiver", readme)
...
""".replace("__CURRENT_OPTIONS__", current_options).replace(
    ...
    def test_camera_hub_state_authority_stays_in_powershell_and_public_status_is_supervisor_only(self) -> None:
        server = read_launcher_server()
        app = read_public("app.js")
        stack_start = read_stack_start_script()
        stack_status = read_stack_status_script()

        for source in (stack_start, stack_status):
        ...
        for legacy_symbol in (
            "CAMERA_HUB_STATE_FILE",
            "CAMERA_STATE_CLASSES",
            "invalidCameraHubState",
            "cameraHubRegistryEntryState",
            "inspectCameraHubRuntimeOwnership",
            "cameraHubManifestState",
            "cameraHubServiceState",
            "camera_state_validation_class",
            "camera_state_operational",
        ):
            self.assertNotIn(legacy_symbol, server)
        ...
        self.assertNotIn("camera_state_", status_projection)
        ...
        self.assertIn("camera_registry_validation_failed", stack_status)
        self.assertIn("camera_manifest_runtime_ownership_invalid", stack_start)
        self.assertIn("-StateOverride $mediapipeState", stack_status)
        self.assertNotIn("ready_detail", stack_status)

        self.assertIn("redactCameraSelectionArgument", server)
        self.assertIn("redactCameraSelectionInCommandText", server)
