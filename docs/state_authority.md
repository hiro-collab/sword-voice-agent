...
| Launcher stopped private-plan artifact observation | Node Launcher private-plan owner + runtime | exact plan-path `lstat`; process-local public/diagnostic view | persisted `stopped/clear` の再Stopだけをmetadata観測する。exact not-foundは`absent/already_stopped`。regular file=`present`、reparse/non-file=`invalid`、access/other error=`unavailable`はpersisted bytes/revisionを変えずpublic/current viewだけ`stopped/cleanup=unknown`、result=`terminal_unknown`へ落とす。content read、removal、lease、worker、dispatch、takeover、retryは0 |
| Launcher Start conflict/cancellation | Node Launcher runtime + reducer | existing `launcher_operation.v2` and process-local cancellation fence | positive join は無効。重複Startはmutation0/dispatch0、Start dispatch timeoutは`start_dispatch_unknown/terminal_unknown/retry0`。Stop-during-Startは同一operation/generation/lease/clientだけがS2 cleanupを実行し、不明authorityはside-effect0/replacement worker0。cancel fieldや別authorityは永続化しない |
| Launcher owner diagnostics | Node Launcher runtime; existing server log adapter | bounded rotating `launcher-stack.log` | fixed owner/boundary/phase/reason/generation/revision/terminal/side-effect/cleanup/retry classのみ。`private_plan_adapter` attributionは許可するが、artifact bytes/hash/path/payload、raw command/PID/portは出さない。sink failureはlifecycle truthを変えない。live ACL/retentionは別proof |
| Launcher private Home config path | Node Launcher server/compiler | saved/default private config and private plan only | `HomeControlConfigPath` はserver-owned。request/UIは上書き権限0。public state/status/preview/save/logはopaque config identity/revisionと固定`homeControlConfigState`のみを出し、raw root/path/command/private identifierは0 |
