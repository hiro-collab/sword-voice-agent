...
| `OpenAIBrokerRequestBudget` | Launcher saved canonical config | gitignored `launcher-config.json`, `effective_config_sha256`, and sealed private broker argv | integer `1..64`, missing defaults to `64`, Checkpoint A explicitly selects `2`; invalid input fails without mutation. The broker enforces exhaustion before secret/upstream access, but its cumulative count is not authorship evidence |
