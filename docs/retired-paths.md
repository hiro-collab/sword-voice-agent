...
| `mediapipe-sword-sign\apps\serve_websocket.py` | Control Plane / Launcher からの旧 direct JSON WebSocket 起動は退役済み。organ 側の残存ファイルは `legacy-delete-candidate` で、Camera Hub topic envelope への置換、非archive consumer 0、Git 回復可能性、organ の focused test が揃った時点で削除する。 |
