...
$opsScriptRoot = Join-Path $WorkspaceRoot "sword-voice-agent\ops\scripts"
...
set "TARGET=%WORKSPACE_ROOT%\sword-voice-agent\ops\scripts\system.ps1"
...
setlocal
for %%I in ("%~dp0.") do set "WORKSPACE_ROOT=%%~fI"
set "TARGET=%WORKSPACE_ROOT%\sword-voice-agent\ops\scripts\home-control-stack\$ScriptName"
where pwsh >nul 2>nul
if %ERRORLEVEL%==0 (
...
`$Target = Join-Path `$WorkspaceRoot "sword-voice-agent\ops\scripts\system.ps1"
...
`$ErrorActionPreference = "Stop"
`$WorkspaceRoot = (Resolve-Path -LiteralPath (Join-Path `$PSScriptRoot "..")).Path
`$Target = Join-Path `$WorkspaceRoot "sword-voice-agent\ops\scripts\home-control-stack\$ScriptName"
